...
export default function Products() {
  ...
  const currentProducts = extendedProducts.slice(
    startIndex,
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {currentProducts.map((product, idx) => (
              <div
                ...
                className="group bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-500 cursor-pointer border border-gray-100"
                data-aos="zoom-in"
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      {product.colors.map((color, colorIndex) => (
                        <div
                          key={colorIndex}
                          className={`h-5 w-5 rounded-full ${colorClasses[color]} border border-gray-300`}
                        />
                      ))}
                    </div>
                    <button
